...
import type { UueCodecService } from "@/services/UueCodecService";
...
> {
  ...
  public async parseDocumentAndFormatOutput(
    ...
    if (companyName || cik || ticker) {
      (parsedDocument as any).companyConformedName = companyName;
      (parsedDocument as any).cik = cik;
      (parsedDocument as any).ticker = ticker;
    }
